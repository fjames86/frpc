...
(defun write-xdr-object (stream spec object)
...
"
  ...
  (cond
    ((symbolp spec)
     (write-xtype spec stream object))
    ((eq (car spec) :array)
     (destructuring-bind (sub-spec length) (cdr spec)
       (dotimes (i length)
	 (write-xdr-object stream sub-spec (aref object i)))))
    ((eq (car spec) :varray)
     (destructuring-bind (sub-spec &optional length) (cdr spec)
       (declare (ignore length))
       (let ((len (length object)))
	 (write-uint32 stream len)
	 (dolist (obj object)
	   (write-xdr-object stream sub-spec obj))
	 ;; write the padding if needed
	 (when (eq sub-spec :octet)
	   (write-array-padding stream len)))))
    ((eq (car spec) :varray*)
     (destructuring-bind (sub-spec &optional length) (cdr spec)
       (declare (ignore length))
	 ...
	 (when (eq sub-spec :octet)
	   (write-array-padding stream len)))))
    ((eq (car spec) :union)
     (destructuring-bind (enum-type &rest arms) (cdr spec)
       (let ((tag (xunion-tag object))
	     (val (xunion-val object)))
	 (write-uint32 stream (enum enum-type tag))
	 (let ((arm (or (assoc tag arms)
			(assoc 'otherwise arms))))
	   (when arm
	     (write-xdr-object stream (cadr arm) val))))))
    ((eq (car spec) :struct)
     (destructuring-bind (name &rest slots) (cdr spec)
       (dolist (slot slots)
	 (destructuring-bind (sname stype &rest options) slot 
	   (declare (ignore options))
	   ;; find the accessor for the slot 
	   (let ((accessor 
		  (symbol-function 
		   (find-symbol (format nil "~A-~A" name sname) 
				(symbol-package name)))))
    ...
    ((eq (car spec) :list)
     (let ((specs (cdr spec)))
       (do ((sub-spec specs (cdr sub-spec))
	    (list object (cdr list)))
	   ((or (null sub-spec) (null list)))
	 (write-xdr-object stream (car sub-spec) (car list)))))
    ((eq (car spec) :plist)
     (do ((slots (cdr spec) (cddr slots)))
	 ((null slots))
       (write-xdr-object stream 
			 (cadr slots) 
...
(defun read-xdr-object (stream spec)
...
"
  ...
  (cond
    ((symbolp spec)
     (read-xtype spec stream))
    ((eq (car spec) :array)
     (destructuring-bind (sub-spec length) (cdr spec)
       (let ((arr (make-array length)))
	 (dotimes (i length)
	   (setf (aref arr i)
		 (read-xdr-object stream sub-spec)))
	 arr)))
    ((eq (car spec) :varray)
     (destructuring-bind (sub-spec &optional length) (cdr spec)
       (declare (ignore length))
       (let ((len (read-uint32 stream)))
	 (prog1 (loop :for i :below len :collect 
		   (read-xdr-object stream sub-spec))
	   (when (eq sub-spec :octet)
	     (read-array-padding stream len))))))
    ((eq (car spec) :varray*)
     (destructuring-bind (sub-spec &optional length) (cdr spec)
       (declare (ignore length))
       (let ((len (read-uint32 stream)))
    ...
    ((eq (car spec) :union)
     (destructuring-bind (enum-type &rest arms) (cdr spec)
       (let ((tag (enum enum-type (read-uint32 stream))))
	 (let ((arm (or (assoc tag arms)
			(assoc 'otherwise arms))))
	   (when arm
	     (read-xdr-object stream (cadr arm)))))))
    ((eq (car spec) :struct)
     (destructuring-bind (name &rest slots) (cdr spec)
	      ...
	      (mapcan (lambda (slot)
			(destructuring-bind (sname stype &rest options) slot
			  ...
			  (list (intern (symbol-name sname) :keyword)
				(read-xdr-object stream stype))))
		      slots))))
    ((eq (car spec) :list)
     (loop :for sub-spec :in (cdr spec)
	:collect (read-xdr-object stream sub-spec)))
    ((eq (car spec) :plist)
     (do ((specs (cdr spec) (cddr specs))
	  (obj nil))
	 ((null specs) obj)
